	...
	<div id="wrapper">
		...
		<div id="page-wrapper">
									...
									<tbody>
										...
										<c:forEach var="item" items="${projects}" varStatus="index">
											<tr>
												<td>${index.count}</td>
												<td>${item.name}</td>
												<td>${item.startDate.toLocalDate()}</td>
												<td>${item.endDate.toLocalDate()}</td>
												<td>${item.leader.firstName} ${item.leader.lastName}</td>
												<td><a href="project-edit?id=${item.id }"
													class="btn-edit btn btn-sm btn-primary">Sửa</a> <a
													id="${item.id}" class="btn-xoa btn btn-sm btn-danger">Xóa</a>
			...
			<div id="confirm" class="modal">
				<div class="modal-body">Bạn có chắc muốn xóa dự án?</div>
